...
# 加载模型
model = DetectMultiBackend('D:/yolov5/eye.pt')
...
def process_img(original_image, select=0):
    ...
    if select == 0:
        LH, LS, LV, HH, HS, HV = 0, 0, 77, 179, 255, 95  #眼眶
    else:
        LH, LS, LV, HH, HS, HV = 0, 0, 0, 179, 255, 32   #眼珠
    #用颜色分割图像
...
def tana(yk, yz):   #输入眼眶和眼珠中心点
    ...
    return (int(yk[0] + zx * 50), int(yk[1] + zy * 50))  #根据斜率，返回一个z轴投影到相应位置的点
...
csv_writer.writerow(['Frame', 'tanxy_x', 'tanxy_y'])
    ...
    for i, det in enumerate(pred):
        ...
        if len(det):
            ...
            if len(det) > 0:
                ...
                # 保存放大的图像
                save_image(roi, f'HSV\detected_object_frame_{frame_count}.png')
                ...
                for cnt in contours:
                    ...
                    yk_xy = (x1 + int(w1 / 2), y1 + int(h1 / 2))
                    cv2.rectangle(roi, (x1, y1), (x1 + w1, y1 + h1), (255, 0, 0), 2)
                    cv2.line(roi, (x1 + int(w1 / 2), 0), (x1 + int(w1 / 2), rows), (0, 255, 0), 2)
                    cv2.line(roi, (0, y1 + int(h / 2)), (cols, y1 + int(h1 / 2)), (0, 255, 0), 2)
                    break
