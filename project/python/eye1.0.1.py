...
from eye import LH,LS,LV,HH,HS,HV
...
# 定义处理图像的函数
def process_img(original_image,LH,LS,LV,HH,HS,HV, select=0):
...
def resize_image(image, scale_percent):
    ...
    dim = (width, height)
    return cv2.resize(image, dim, interpolation=cv2.INTER_AREA)  # 使用 cv2.resize 函数根据计算出的尺寸 dim 来缩放图像，
    # interpolation=cv2.INTER_AREA 参数指定了缩放时使用的插值方法，这里使用的是区域插值，它通常用于图像的缩放。
        ...
        if len(det):
            ...
            if len(det) > 0:
                ...
                for *xyxy, conf, cls in det:
                    ...
                    cv2.putText(frame, label, (int(xyxy[0]), int(xyxy[1]) - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.3,
                                (0, 255, 0), 2)
                ...
                threshold = process_img(roi,LH,LS,LV,HH,HS,HV)
                ...
                if 'y1' in locals() and 'x1' in locals():
                    ...
                    eye_img1 = process_img(eye_img1,LH,LS,LV,HH,HS,HV, select=1)
